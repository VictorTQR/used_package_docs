---
title: "FilesCard 文件卡片 📇 | Element-Plus-X"
source: "https://element-plus-x.com/zh/components/filesCard/"
author:
...
设置 name 属性, 且 name 没有后缀。name="测试文件" 设置 name 属性，有文件后缀。name="测试文件.pdf" 支持更据 name 后缀匹配内置图标 如果有后缀，但是匹配不到常用的图标，则默认为 File 文件

[基础用法](https://element-plus-x.com/zh/components/filesCard/#zh-components-filesCard-demos-base)

你可以在 组件实例上拿到 colorMap 内置文件类型 fileType: color 对象。 内置了 16 种文件类型图标。

### 状态设置

设置 status 属性，控制文件加载状态 "uploading","done","error" "uploading"+"percent" 控制上传进度，"error"+"errorTip"控制自定义失败提示

[status 和 percent 属性](https://element-plus-x.com/zh/components/filesCard/#zh-components-filesCard-demos-status)

控制文件加载状态（上传中、完成、失败）及进度显示。

### 展示删除图标

[删除图标、删除事件](https://element-plus-x.com/zh/components/filesCard/#zh-components-filesCard-demos-delete-icon)

可以设置 `showDelIcon` 属性来显示删除图标，并设置 `@delete` 方法来设置删除事件。

### 图片文件展示

图片文件 可预览 和 不可预览 图片文件 正方形变体 其他格式不受变体属性影响 图片文件 默认长方形变体 支持上传状态 、支持预览开启关闭 、支持预览遮罩蒙层开启关闭 图片文件 正方形变体 支持上传状态 、支持预览开启关闭 、支持预览遮罩蒙层开启关闭

[图片文件专区](https://element-plus-x.com/zh/components/filesCard/#zh-components-filesCard-demos-image-preview)

支持图片预览、正方形/长方形变体、上传状态覆盖层等功能。同样也可以 通过 status 和 percent 控制。

### 自定义样式与交互

[定制样式](https://element-plus-x.com/zh/components/filesCard/#zh-components-filesCard-demos-custom-style)
...
[自定义内置文件对应的颜色](https://element-plus-x.com/zh/components/filesCard/#zh-components-filesCard-demos-custom-color)
